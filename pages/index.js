...
export default function Home() {
  ...
  async function fetchListing() {
    ...
    try {
      ...
      const imgs = (d.images || []).filter(u => u && u.startsWith('http'));
      setAllImgs(imgs);
      if (imgs.length) {
        setStatus({ msg: `✓ ${d.title || 'Vehicle'} — ${imgs.length} photos loaded. Select below.`, type: 'ok' });
      } else {
        setStatus({ msg: `✓ ${d.title || 'Data loaded'} — no images auto-found. Use Paste URLs tab.`, type: 'warn' });
      }
    } catch (e) {
  ...
  function loadPasted() {
    ...
    setAllImgs(urls.slice(0, 30)); setSelImgs([]);
    setImgTab('auto');
  ...
  async function doExport() {
    ...
    const pdf = new jsPDF({ unit: 'px', format: [360, 800], hotfixes: ['px_scaling'] });
    let first = true;
    for (const slide of slides) {
      const c = await html2canvas(slide, { scale: 2, useCORS: true, allowTaint: true, backgroundColor: '#000', logging: false });
      const img = c.toDataURL('image/jpeg', 0.93);
      const pw = 360, ph = Math.round((c.height / c.width) * pw);
      if (!first) pdf.addPage([360, ph]); else { pdf.deletePage(1); pdf.addPage([360, ph]); }
      first = false;
  ...
  const feats = fields.feats.split('\n').filter(f => f.trim()).map(f => { const p = f.split(':'); return { b: p[0]?.trim() || '', d: p.slice(1).join(':').trim() }; });
  ...
  const LBOT = `<div style="display:flex;flex-direction:column;align-items:center;padding:16px;gap:2px">${MSML}<div style="font-size:12px;font-weight:900;color:#2563eb;letter-spacing:2px;text-align:center">EASY AUTO LOANS<br/>CANADA</div></div>`;
  const STAR = (items) => items.map(t => `<div style="display:flex;align-items:center;gap:10px;font-size:18px;font-weight:700;color:white"><span style="color:#2563eb;font-size:20px">&#9733;</span> ${t}</div>`).join('');

  const slides_html = !preview ? null : [
    // Cover
    `<div style="aspect-ratio:9/16;display:flex;flex-direction:column;align-items:center;justify-content:center;background:#000;padding:30px">${MAPLE}<div style="font-size:22px;font-weight:900;color:#2563eb;letter-spacing:2px;text-transform:uppercase;text-align:center;line-height:1.15;margin-top:-4px">EASY AUTO LOANS<br/>CANADA</div><div style="margin-top:40px;text-align:center"><div style="display:flex;align-items:center;justify-content:center;gap:7px;font-size:19px;font-weight:900;color:white"><span style="color:#2563eb;font-size:22px">&#9733;</span>#1</div><div style="font-size:15px;font-weight:800;color:white;text-align:center;line-height:1.3">Auto Finance Company<br/>in the Country</div></div></div>`,
    // Unit Description
    `<div style="background:linear-gradient(180deg,#050510,#0a0a18);min-height:500px">${HDR('Unit Description')}<div style="padding:0 16px;font-size:19px;font-weight:900;color:white">${fields.title}</div><div style="padding:0 16px;font-size:17px;font-weight:800;color:white;margin-bottom:10px">${fields.color} ${fields.kms}</div><div style="padding:0 12px 16px;display:flex;flex-direction:column;gap:6px">${feats.map(f=>`<div style="display:flex;gap:9px;align-items:flex-start;font-size:13px;line-height:1.4;color:white"><div style="width:6px;height:6px;background:white;border-radius:50%;margin-top:5px;flex-shrink:0"></div><div><strong>${f.b}${f.d?' –':''}</strong> <span style="color:#bbb">${f.d}</span></div></div>`).join('')}</div></div>`,
    // Financial
    `<div style="background:linear-gradient(180deg,#050510,#0a0a18)">${HDR('Financial Breakdown')}<div style="padding:16px;display:flex;flex-direction:column;gap:11px"><div style="font-size:22px;font-weight:800;color:white">Was: ${fields.wasP}</div><div style="font-size:25px;font-weight:900;color:white">Today: ${fields.todP}</div><div style="font-size:22px;font-weight:800;color:white">Savings: ${fields.savP}</div><hr style="border:none;border-top:1px solid #1a1a2e"/><div><div style="font-size:22px;font-weight:800;color:white">${fields.downP} down</div><div style="font-size:28px;font-weight:900;color:#22c55e">${fields.bwP}</div><div style="font-size:20px;font-weight:800;color:white">Payment is all in!!!</div><div style="font-size:23px;font-weight:900;color:#22c55e;margin-top:12px;line-height:1.3">Qualify for an upgrade in<br/>${upg}!</div></div></div></div>`,
    // Benefits
    ...
    // Photos
    ...pairs.map(p => p.length === 2
      ? `<div style="aspect-ratio:9/16;display:flex;flex-direction:column"><img src="${p[0]}" style="flex:1;width:100%;object-fit:cover;display:block"/><div style="height:4px;background:#000;flex-shrink:0"></div><img src="${p[1]}" style="flex:1;width:100%;object-fit:cover;display:block"/></div>`
      : `<div style="aspect-ratio:9/16"><img src="${p[0]}" style="width:100%;height:100%;object-fit:cover;display:block"/></div>`
    ),
    // Delivery
      ...
      <div style={{ display:'flex', height:'100vh', fontFamily:'Segoe UI, system-ui, sans-serif', fontSize:13, background:'#0d0d12', color:'#e2e8f0' }}>
        ...
        <div style={{ width:350, minWidth:350, background:'#13131a', borderRight:'1px solid #1e1e2c', overflowY:'auto', padding:16 }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:16 }}>
            <div style={{ width:32, height:32, background:'#2563eb', borderRadius:7, display:'flex', alignItems:'center', justifyContent:'center', fontWeight:900, fontSize:14, color:'white' }}>T</div>
            <div style={{ fontWeight:700, fontSize:15 }}>Vehicle Presentation Builder <span style={{ color:'#64748b', fontSize:11 }}>by TYME</span></div>
          </div>

          <Label>Listing URL — GoAuto · AutoTrader · CarGurus</Label>
          <div style={{ display:'flex', gap:6 }}>
            <Input value={url} onChange={e => setUrl(e.target.value)} placeholder="Paste listing URL..." onKeyDown={e => e.key === 'Enter' && fetchListing()} style={{ flex:1, width:'auto' }} />
          ...
          <div style={{ marginBottom:8 }}><FLabel>Year / Make / Model / Trim</FLabel><Input value={fields.title} onChange={e => sf('title', e.target.value)} placeholder="2020 Kia Sorento EX" /></div>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, marginBottom:8 }}>
            <div><FLabel>Colour</FLabel><Input value={fields.color} onChange={e => sf('color', e.target.value)} placeholder="Grey" /></div>
            <div><FLabel>Kilometres</FLabel><Input value={fields.kms} onChange={e => sf('kms', e.target.value)} placeholder="115,000 kms" /></div>
          </div>

          <HR />
          <Label>Deal Numbers</Label>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, marginBottom:8 }}>
            <div><FLabel>Was Price</FLabel><Input value={fields.wasP} onChange={e => sf('wasP', e.target.value)} placeholder="$27,995" /></div>
            <div><FLabel>Today&apos;s Price</FLabel><Input value={fields.todP} onChange={e => sf('todP', e.target.value)} placeholder="$25,700" /></div>
          </div>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, marginBottom:8 }}>
            <div><FLabel>Savings</FLabel><Input value={fields.savP} onChange={e => sf('savP', e.target.value)} placeholder="$2,295" /></div>
            <div><FLabel>Down Payment</FLabel><Input value={fields.downP} onChange={e => sf('downP', e.target.value)} placeholder="$3,000" /></div>
          </div>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, marginBottom:8 }}>
            <div><FLabel>Biweekly Payment</FLabel><Input value={fields.bwP} onChange={e => sf('bwP', e.target.value)} placeholder="$283 biweekly" /></div>
            <div><FLabel>Upgrade In</FLabel><Input value={fields.upgP} onChange={e => sf('upgP', e.target.value)} placeholder="10 months" /></div>
          </div>

          <HR />
          <Label>Key Features <span style={{ color:'#64748b', fontSize:9, fontWeight:400, textTransform:'none', letterSpacing:0 }}>Bold: description, one per line</span></Label>
          <textarea value={fields.feats} onChange={e => sf('feats', e.target.value)} placeholder={"3.3L V6 Engine: Strong reliable power\nAWD: Traction in snow and rain\n7-Passenger Seating: Room for the whole family\nHeated Seats & Steering: Built for Canadian winters\nPush-Button Start: Keyless convenience\nDual-Zone Climate: Individual temperature control\nWireless Charging: No cords needed\nPower Lumbar Seat: 8-way adjustable"} style={{ width:'100%', background:'#0f0f16', border:'1px solid #1e1e2c', borderRadius:6, padding:'8px 10px', color:'#e2e8f0', fontSize:12, fontFamily:'inherit', resize:'vertical', minHeight:80, outline:'none', marginBottom:8 }} />

          <HR />
          <Label>Photos</Label>
          <div style={{ display:'flex', border:'1px solid #1e1e2c', borderRadius:6, overflow:'hidden', marginBottom:8 }}>
            {['auto','paste'].map(t => <button key={t} onClick={() => setImgTab(t)} style={{ flex:1, padding:7, fontSize:11, fontWeight:600, cursor:'pointer', border:'none', background:imgTab===t?'#2563eb':'#0f0f16', color:imgTab===t?'white':'#64748b', transition:'all .12s' }}>{t === 'auto' ? 'Auto (from Pull)' : 'Paste URLs'}</button>)}
          </div>
          ...
          {imgTab === 'auto' && <>
            ...
            {allImgs.length === 0
              ...
              : <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:4 }}>
                  {allImgs.map(src => (
                    <div key={src} onClick={() => toggleImg(src)} style={{ aspectRatio:'4/3', borderRadius:4, overflow:'hidden', cursor:'pointer', border:`2px solid ${selImgs.includes(src)?'#2563eb':'transparent'}`, position:'relative', background:'#1a1a2e' }}>
        ...
        <div style={{ flex:1, overflowY:'auto', background:'#111', display:'flex', flexDirection:'column', alignItems:'center', padding:'20px 16px' }} ref={prevRef}>
          {!preview
            ...
            : slides_html.map((html, i) => (
                <div key={i}>
                  <div style={{ fontSize:9, color:'#64748b', marginBottom:2, alignSelf:'flex-start', letterSpacing:.3 }}>SLIDE {i+1}</div>
                  <div className="slide" style={{ width:360, background:'#000', color:'white', overflow:'hidden', marginBottom:8, borderRadius:2, boxShadow:'0 3px 16px rgba(0,0,0,.8)', flexShrink:0 }} dangerouslySetInnerHTML={{ __html: html }} />
                </div>
              ))
          }
        </div>
...
const Input = ({ style, ...p }) => <input {...p} style={{ background:'#0f0f16', border:'1px solid #1e1e2c', borderRadius:6, padding:'8px 10px', color:'#e2e8f0', fontSize:12, outline:'none', width:'100%', fontFamily:'inherit', ...style }} />;
...
const Label = ({ children }) => <div style={{ fontSize:9, fontWeight:700, letterSpacing:'1.5px', textTransform:'uppercase', color:'#64748b', margin:'14px 0 6px' }}>{children}</div>;
const FLabel = ({ children }) => <div style={{ fontSize:10, color:'#64748b', fontWeight:600, letterSpacing:.3, marginBottom:3 }}>{children}</div>;
