...
function CropModal({ src, onDone, onCancel }) {
  ...
  function onMouseDown(e) {
    ...
    for (const [hk, hv] of Object.entries(handles)) {
      if (Math.abs(p.x - hv.x) < 0.04 && Math.abs(p.y - hv.y) < 0.04) {
        setDrag({ type: 'handle', handle: hk, start: p, origCrop: { ...crop } });
        return;
      }
    }
    // Check if inside crop — move
    if (p.x > crop.x && p.x < crop.x + crop.w && p.y > crop.y && p.y < crop.y + crop.h) {
      setDrag({ type: 'move', start: p, origCrop: { ...crop } });
    }
  }
  ...
  function drawCanvas(c) {
    ...
    for (const hv of Object.values(getHandles(c))) {
      ctx.beginPath();
      ctx.arc(hv.x * W, hv.y * H, 6, 0, Math.PI * 2);
      ctx.fill();
    }
  }
  ...
  function applyCrop() {
    ...
    out.width = Math.round(crop.w * img.naturalWidth);
    out.height = Math.round(crop.h * img.naturalHeight);
    const ctx = out.getContext('2d');
    ctx.drawImage(img, crop.x * img.naturalWidth, crop.y * img.naturalHeight, out.width, out.height, 0, 0, out.width, out.height);
...
export default function Home() {
  ...
  const [cropImg, setCropImg] = useState(null); // {src, index}
  const [pasteUrls, setPasteUrls] = useState('');
  const [imgTab, setImgTab] = useState('auto');
  const [preview, setPreview] = useState(false);
  const [exporting, setExporting] = useState(false);
  const [erasing, setErasing] = useState(null); // index being erased
  ...
  // Crop: open modal for selected image
  function openCrop(src, idx) {
    setCropImg({ src: proxyImg(src), origSrc: src, idx });
  }

  function applyCrop(dataUrl) {
    const { origSrc, idx } = cropImg;
    // Replace in allImgs
    setAllImgs(prev => prev.map((s, i) => {
      const si = selImgs.indexOf(s);
      // Find which allImgs index this selected image corresponds to
      return s;
    }));
    // Replace in selImgs
    setSelImgs(prev => prev.map((s, i) => i === idx ? dataUrl : s));
    // Also update allImgs if it was there
    setAllImgs(prev => prev.map(s => s === origSrc ? dataUrl : s));
    setCropImg(null);
  }

  // AI Logo Eraser using remove.bg
  async function eraseBackground(src, selIdx) {
    setErasing(selIdx);
    try {
      // First get the image as blob via proxy
      ...
      if (!r.ok) throw new Error('Remove.bg failed: ' + r.status);
      ...
      setStatus({msg:'✓ Background removed successfully',type:'ok'});
    } catch(e) {
      setStatus({msg:'Logo removal failed: '+e.message,type:'err'});
    }
    setErasing(null);
  }
          ...
          {imgTab==='auto' && <>
            ...
            {allImgs.length===0
              ...
              : <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:4}}>
                {allImgs.map((src,i)=>{
                  const isSelected = selImgs.includes(src);
                  const selIdx = selImgs.indexOf(src);
                  return (
                    <div key={i} style={{position:'relative',borderRadius:4,overflow:'hidden',border:`2px solid ${isSelected?'#2196f3':'transparent'}`,background:'#1a1a2e'}}>
                      ...
                      {isSelected && <>
                        ...
                        <div style={{display:'flex',gap:2,padding:'2px 3px',background:'rgba(0,0,0,0.7)'}}>
                          <button onClick={()=>openCrop(src,selIdx)} title="Crop" style={{flex:1,background:'#1e40af',border:'none',borderRadius:3,color:'white',fontSize:9,cursor:'pointer',padding:'2px 0'}}>✂ Crop</button>
                          <button onClick={()=>eraseBackground(src,selIdx)} title="Remove logo/background" disabled={erasing===selIdx} style={{flex:1,background:'#7c3aed',border:'none',borderRadius:3,color:'white',fontSize:9,cursor:'pointer',padding:'2px 0'}}>
                            {erasing===selIdx?'..':'🪄 AI'}
                          </button>
                        </div>
                      </>}
            ...
            {selImgs.length>0 && <div style={{fontSize:9,color:'#64748b',marginTop:2}}>✂ Crop · 🪄 AI removes backgrounds & dealer logos</div>}
          </>}
