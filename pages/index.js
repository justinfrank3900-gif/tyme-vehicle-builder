...
// Parse ChatGPT bullet text into array
...
// PandaDoc SVG logo
const PANDADOC_SVG = `<svg viewBox="0 0 120 36" xmlns="http://www.w3.org/2000/svg" style="width:140px;height:42px">
  <rect width="36" height="36" rx="6" fill="#3CBB70"/>
  <text x="7" y="27" font-family="Arial,sans-serif" font-weight="900" font-size="22" fill="white">pd</text>
  <text x="44" y="26" font-family="Arial Black,sans-serif" font-weight="900" font-size="18" fill="white">PandaDoc</text>
</svg>`;
  ...
  const slides_html = !preview ? null : [
    ...
    `<div style="background:linear-gradient(180deg,#050510,#0a0a18);padding-bottom:20px">
      ...
      <div style="padding:0 14px;display:flex;flex-direction:column;gap:10px">
        ${features.map(f => `<div style="display:flex;gap:12px;align-items:flex-start;font-size:16px;line-height:1.5;color:white">
          <div style="width:8px;height:8px;min-width:8px;background:#2196f3;border-radius:50%;margin-top:6px;box-shadow:0 0 6px rgba(33,150,243,0.8)"></div>
          <div>${f}</div>
        </div>`).join('')}
      </div>
    </div>`,
    ...
    `<div style="background:linear-gradient(180deg,#050510,#0a0a18);padding-bottom:20px">
      ...
      <div style="padding:0 16px;display:flex;flex-direction:column;gap:12px">
        ...
        ${fields.trade || fields.lien || fields.equity ? `<div style="border-top:1px solid #1e1e2e;padding-top:10px;display:flex;flex-direction:column;gap:8px">
          ${fields.trade ? `<div style="font-size:22px;font-weight:800;color:white">Trade: ${fields.trade}</div>` : ''}
          ${fields.lien ? `<div style="font-size:22px;font-weight:800;color:white">Lien: ${fields.lien}</div>` : ''}
          ${fields.equity ? `<div style="font-size:22px;font-weight:800;color:white">Neg Equity: ${fields.equity}</div>` : ''}
        </div>` : ''}
    ...
    <>
      ...
      <div style={{ display: 'flex', height: '100vh', background: '#0d0d12', color: '#e2e8f0', fontSize: 13 }}>
        ...
        <div style={{ width: 355, minWidth: 355, background: '#13131a', borderRight: '1px solid #1e1e2c', overflowY: 'auto', padding: 16 }}>
          ...
          />
          {parseBullets(featText).length > 0 && (
            <div style={{ fontSize: 10, color: '#86efac', marginBottom: 6 }}>✓ {parseBullets(featText).length} bullets detected</div>
          )}
